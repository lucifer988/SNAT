...
一个用于集中管理多台 Linux 节点 **TCP 端口转发** 的 Web 面板，提供规则下发、流量统计、状态对账、告警与审计的一体化管理。
...
- [转发到内网目标](#转发到内网目标)
- [流量限额](#流量限额)
- [本地 UI 验收 / 假 Agent](#本地-ui-验收--假-agent)
- [状态说明](#状态说明)
- [Telegram 告警与命令](#telegram-告警与命令)
- [推荐网络架构](#推荐网络架构)
- [安全默认值](#安全默认值)
- [更新与旧版迁移](#更新与旧版迁移)
- [测试](#测试)
...
- 多 Agent 节点统一管理，服务器列表支持 **拖拽排序**（按住彩色三横线把手拖动，不改变服务器 ID）
- TCP 端口转发规则新增、编辑、启停和删除；规则树支持搜索、排序与 **手动拖拽排序**（不改变 TGBOT 规则编号）
- 批量启用 / 禁用 / 删除，规则对账与重新下发
- 流量统计、活跃连接数、单规则流量限额（Agent 本地执行，Web 离线仍生效）
- 域名目标定时解析与更新
- 规则快照、导入导出、备份恢复和审计日志
- Telegram 离线 / 限额 / 审计告警，可选 Telegram 命令（默认关闭）
- Agent HMAC 签名认证、nonce 防重放、来源 IP 白名单
- 会话吊销一致性、请求头净化、Agent 恢复期目标策略校验等纵深防御
- 失败回滚、删除确认和 `desynced` / `unknown` / `suspended` 状态提示
- 可选 Prometheus `/metrics`（需配置 `SNAT_METRICS_TOKEN`）
...
update.sh              固定提交 SHA 更新与回滚
uninstall_web.sh       卸载 Web 面板
uninstall_agent.sh     卸载 Agent
wireguard_setup.sh     WireGuard 辅助脚本
wg_peer_helper.sh      WireGuard peer 注册辅助脚本
wg_peer_worker.py      WireGuard peer 注册后台任务
migrate_db.sh          旧版数据迁移
show_token.sh          查看安装生成的 Token
docker-compose.yml     Docker Compose 部署
Dockerfile.web         Web 镜像
Dockerfile.agent       Agent 镜像
fake_agent.py          本地验收用假 Agent
DEPLOY.md              完整部署与加固说明
CHANGES.md             变更记录
tests_smoke.py         基础测试
verify_e2e.py          Agent 认证 E2E 测试
tests_round7.py        状态一致性与部署回归测试
tests_security_round8.py  安全回归测试
tests_round10_hardening.py 第十轮加固回归测试
tests_security_round11.py 第十一轮安全回归测试
tests_wireguard.py     WireGuard 测试
tests_wireguard_enrollment.py WireGuard 注册测试
```
...
至少设置以下强随机值（可用 `openssl rand -hex 32` 生成）：
...
公网部署还支持一次性注册码（`WG_ENROLL_TOKEN`）自动接入 Agent，详见 [DEPLOY.md](./DEPLOY.md)。

## 安全默认值

- `AGENT_ALLOW_BEARER=0`：默认仅接受 HMAC 签名请求
- `AGENT_SET_FORWARD_POLICY_ACCEPT=0`：不修改宿主机全局 FORWARD 策略，为每条托管规则添加精确放行项
...
- 强制 HTTPS 时，HTTP 直连会被拒绝（`HTTPS required`）
- 会话吊销 / 改密 / 过期后，旧会话与已领取的 CSRF token 立即失效
...
python3 tests_security_round8.py
python3 tests_round10_hardening.py
python3 tests_security_round11.py
