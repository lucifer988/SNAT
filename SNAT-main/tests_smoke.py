...
def build_sig(token, method, path, body=''):
...
class SmokeTest(unittest.TestCase):
    ...
    def test_agent_accepts_signed_request_before_command_execution(self, *_):
        ...
        ts, sig = build_sig(agentapp.TOKEN, 'POST', '/add_rule', body)
        r = self.agent_client.post('/add_rule', data=body, headers={'Authorization': f'Bearer {agentapp.TOKEN}', 'X-Timestamp': ts, 'X-Signature': sig, 'Content-Type': 'application/json'})
        self.assertEqual(r.status_code, 200)
    ...
    def test_agent_get_traffic_uses_forward_counters_for_managed_rules(self):
        ...
        with patch('agent.agent.load_rules', return_value=rules_state), patch('agent.agent.save_rules') as mock_save, patch('agent.agent.run_cmd', side_effect=fake_run):
            ts, sig = build_sig(agentapp.TOKEN, 'GET', '/get_traffic/12345', '')
            r = self.agent_client.get('/get_traffic/12345', headers={'X-Timestamp': ts, 'X-Signature': sig})
    ...
    def test_report_connections_accepts_signed_agent_request(self):
        ...
        body = json.dumps({'server_host': '87.83.105.37', 'samples': [{'local_port': 11111, 'active_connections': 9}]}, separators=(',', ':'))
        ts, sig = build_sig('panel-token', 'POST', '/api/agents/report_connections', body)
        r = self.web_client.post(
            '/api/agents/report_connections',
            data=body,
            headers={'Authorization': 'Bearer panel-token', 'X-Timestamp': ts, 'X-Signature': sig, 'Content-Type': 'application/json'}
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_agent_accepts_signed_get_without_bearer(self, *_):
        ...
        ts, sig = build_sig(agentapp.TOKEN, 'GET', '/list_rules')
        r = self.agent_client.get('/list_rules', headers={'X-Timestamp': ts, 'X-Signature': sig})
        self.assertEqual(r.status_code, 200)
    ...
    def test_agent_rejects_expired_signature(self):
        ...
        r = self.agent_client.get('/list_rules', headers={'X-Timestamp': stale_ts, 'X-Signature': sig})
        self.assertEqual(r.status_code, 401)
    ...
    def test_agent_rejects_tampered_signed_body(self, *_):
        ...
        ts, sig = build_sig(agentapp.TOKEN, 'POST', '/add_rule', signed_body)
