...
def restore_backup():
    ...
    shutil.copy2(db_path, _app.DB_FILE)
...
def import_rules():
    ...
    for row in rows:
        try:
            c.execute(
                ...
                (int(row['server_id']), int(row['local_port']), row.get('target_host', ''), row['target_ip'],
                 int(row['target_port']), row.get('remark', ''), int(row.get('enabled', 1)), int(row.get('traffic_limit_gb', 0)))
            )
            inserted += 1
        except Exception:
            pass
    conn.commit()
    conn.close()
    _app.audit_log('import_rules', 'rules', 'success', f'inserted={inserted}')
