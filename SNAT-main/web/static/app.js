...
function renderServers() {
    ...
    tbody.innerHTML = servers.map(s => {
        ...
        <tr>
            ...
            <td data-label="地址">${s.host}</td>
            <td data-label="端口">${s.port}</td>
            <td data-label="状态" class="status-${s.status}">${label}</td>
            <td data-label="操作" class="col-actions">
                <div class="action-group" style="justify-content:center">
                    <button class="btn-success" onclick="editServer(${s.id})">编辑</button>
                    <button class="btn-primary" onclick="checkServer(${s.id})">检查</button>
                    <button class="btn-danger" onclick="deleteServer(${s.id})">删除</button>
                </div>
            </td>
        </tr>
    `;
    ...
    container.innerHTML = serverNames.map(sn => {
        ...
        return `
            <div class="tree-server">
                <div class="tree-server-header" onclick="toggleTreeGroup('${sn.replace(/'/g, "\\'")}')">
                    <span class="tree-server-icon">✿</span>
                    <span class="tree-server-name">${sn}</span>
                    <span class="tree-server-count">${serverRules.length} 条规则</span>
                    <span class="tree-arrow">▼</span>
                </div>
                <div class="tree-server-rules" id="tree-${sn.replace(/'/g, "_")}">
    ...
    return `
        <div class="tree-rule" style="border-left-color:${borderColor}">
            <div class="tree-rule-main">
                ...
                <span class="tree-rule-target">${targetDisplay}:${r.target_port}</span>
            ...
            <div class="tree-rule-info">
                ...
                ${r.remark ? `<span>备注: ${r.remark}</span>` : ''}
            </div>
...
function renderServerGroups() {
    ...
    container.innerHTML = Object.entries(groups).map(([serverName, serverRules]) => `
        <div class="server-group">
            <div class="server-group-header" onclick="toggleGroup('${serverName.replace(/'/g, "\\'")}')">
                <span class="server-group-title">🖥️ ${serverName}</span>
                <span class="server-group-count">${serverRules.length} 条规则</span>
                <span class="server-group-arrow">▼</span>
            </div>
            <div class="server-group-content" id="group-${serverName.replace(/'/g, "_")}">
    ...
    return `
        <div class="rule-card">
            ...
            <div class="rule-card-row">
                ...
                <span class="rule-card-value">${targetDisplay}:${r.target_port}</span>
            </div>
            <div class="rule-card-row">
            ...
            ${r.remark ? `<div class="rule-card-row"><span class="rule-card-label">备注</span><span class="rule-card-value">${r.remark}</span></div>` : ''}
            <div class="rule-card-actions">
                <button class="btn-primary" onclick="toggleRule(${r.id})">${r.enabled ? '禁用' : '启用'}</button>
                <button class="btn-success" onclick="editRule(${r.id})">编辑</button>
                <button class="btn-danger" onclick="deleteRule(${r.id})">删除</button>
            </div>
        </div>
    `;
}
...
function renderRulesGroup() {
    ...
    container.innerHTML = Object.entries(groups).map(([serverId, g]) => {
        ...
        <div class="server-group" style="margin-bottom:16px;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
            <div class="server-group-header" onclick="toggleGroup(${serverId})" style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;background:linear-gradient(135deg,#667eea,#764ba2);color:white;cursor:pointer;">
                <div>
                    <div style="font-weight:700;font-size:16px;">${g.name}</div>
                    <div style="font-size:12px;opacity:0.9;">${g.host}</div>
                </div>
            ...
            <div class="server-group-rules" id="group_${serverId}" style="display:none;padding:10px;background:#f8fafc;">
                <table style="width:100%;table-layout:auto;">
                    ...
                    <tbody>
                        ${g.rules.map(r => {
                            const targetDisplay = (r.target_host && r.target_host !== r.target_ip) ? `${r.target_host}<br><small style="color:#666">${r.target_ip}</small>` : r.target_ip;
                            return `
                            <tr style="border-bottom:1px solid #eee;">
                                ...
                                <td style="padding:8px;text-align:center;">
                                    <button class="btn-primary" onclick="toggleRule(${r.id})" style="padding:4px 8px;font-size:11px;">${r.enabled ? '禁用' : '启用'}</button>
                                    <button class="btn-success" onclick="editRule(${r.id})" style="padding:4px 8px;font-size:11px;">编辑</button>
                                </td>
                            </tr>
                            `;
                        }).join('')}
...
function renderRules() {
    ...
    tbody.innerHTML = list.map(r => {
        ...
        return `
        <tr>
            <td><input type="checkbox" onchange="toggleRuleSelection(${r.id}, this.checked)" ${selectedRuleIds.has(r.id) ? 'checked' : ''}></td>
            <td class="col-server cell">${r.server_name}</td>
            <td class="col-port cell">${r.local_port}</td>
            <td class="col-ip cell">${(r.target_host && r.target_host !== r.target_ip) ? `${r.target_host} (${r.target_ip})` : r.target_ip}</td>
            <td class="col-port cell">${r.target_port}</td>
            <td class="col-remark cell" title="${r.remark || ''}">${(r.remark || '').slice(0, 12)}</td>
        ...
        rulesList.innerHTML = list.map(r => {
            ...
            <div class="rule-card">
                ...
                <div class="rule-card-row">
                    ...
                    <span class="rule-card-value">${r.server_name}</span>
                </div>
                <div class="rule-card-row">
                    <span class="rule-card-label">目标</span>
                    <span class="rule-card-value">${targetDisplay}:${r.target_port}</span>
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width:600px">
            ...
            <div style="padding:20px">
                ...
                <pre style="background:#f5f5f5;padding:15px;border-radius:5px;overflow-x:auto;word-break:break-all">${cmd}</pre>
...
function showToast(message, color, duration = 3000, detail = '') {
    ...
    toast.innerHTML = `<div style="font-size:18px">${color === '#f5576c' ? '❌' : color === '#ffa500' ? '⚠️' : '✅'}</div><div style="flex:1"><div>${message}</div>${detail ? `<div style="margin-top:6px;font-size:12px;opacity:.9">${detail}</div>` : ''}</div><button style="background:none;border:none;color:#fff;font-size:18px;cursor:pointer">×</button>`;
...
async function showDiagModal() {
    ...
    for (const server of data.servers) {
        ...
        html += `<h3 style="margin:0 0 10px 0;color:${statusColor}">${server.server_name} ${statusText}</h3>`;
        ...
        if (server.status === 'healthy') {
            ...
            html += `<p style="color:#f5576c">错误: ${server.error}</p>`;
        }
        html += '</div>';
    }
    html += '</div>';
...
function showAlert(message) {
    ...
    if (!alertDiv) {
        ...
        div.innerHTML = `<strong>⚠ 告警</strong><br>${message.replace(/\n/g, '<br>')}`;
...
async function bulkCheckServers() {
    ...
    if (data.success) {
        ...
        if (el) el.innerHTML = data.results.map(x => `<div>${x.name}: ${x.ok ? '✅ OK' : '❌ ' + (x.error || ('HTTP ' + (x.status_code ?? 'ERR')))}</div>`).join('');
    }
}
...
async function loadBackups() {
    ...
    if (!el) return;
    el.innerHTML = data.success ? data.items.map(x => `<div>${x.name} <button class="btn-danger" onclick="restoreBackup('${x.path.replace(/'/g, "&#39;")}')">恢复</button></div>`).join('') : '加载失败';
}
...
async function loadSnapshots() {
    ...
    if (!el) return;
    el.innerHTML = data.success ? data.items.map(x => `<div>#${x.id} ${x.created_at} <b>${x.reason || 'manual'}</b> <button class="btn-danger" onclick="restoreSnapshot(${x.id})">恢复</button> <button class="btn-danger" onclick="deleteSnapshot(${x.id})">删除</button></div>`).join('') : '加载失败';
}
...
async function checkAlerts() {
    ...
    if (el) el.innerHTML = data.alerts.map(x => `<div>${x.server}: ${x.ok ? '✅' : '❌ ' + x.detail}</div>`).join('') || '无告警';
}
