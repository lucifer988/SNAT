...
def report_connections():
    ...
    # 主机未录入时也做一次等价开销的比较，抹平"该 host 是否存在"的计时差异（防主机枚举）。
    compare_target = expected_token if expected_token else 'x' * 64
    token_ok = hmac.compare_digest(token or '', compare_target)
