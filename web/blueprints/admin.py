...
@_app.login_required
def export_servers():
    # 默认导出不含 token：即便导出的是密文，也属于敏感配置，不应默认放开、随手外传。
    # 需要含 token 时必须显式 ?include_tokens=1，且会话在最近 REAUTH_MAX_AGE 内二次验证过密码。
    include_tokens = request.args.get('include_tokens', '0').lower() in ('1', 'true', 'yes')
    if include_tokens and not _app._recent_auth_ok():
        return jsonify({'success': False, 'error': '导出含 token 需要重新验证密码', 'reauth_required': True}), 403
    ...
    # 含敏感字段的导出单独审计并标红，方便事后追踪误导出/外传。
    if include_tokens:
        _app.log_event('WARNING', f"⚠ 导出服务器（含 token）: count={len(rows)} by {_app.session.get('username','-')}")
        _app.audit_log('export_servers_with_tokens', 'servers', 'success', f'count={len(rows)}')
    else:
        _app.audit_log('export_servers', 'servers', 'success', f'count={len(rows)} tokens=redacted')
